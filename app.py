import matplotlib
matplotlib.use('Agg')  # Indispensable para Render/Linux
...
def generar_visualizaciones(n_clusters):
    # Usar ruta absoluta basada en la ubicación del archivo app.py
    ...
    # Datos sintéticos para simulación coherente
    np.random.seed(42)
    ...
    plt.scatter(X_vis[:, 0], X_vis[:, 1], c=COLOR_TEXTO, s=2, alpha=0.3)
...
def index():
    ...
    # Generar imágenes nuevas en cada carga
    generar_visualizaciones(n_clusters)

    # Datos específicos para que NO salgan en cero o iguales
    # Se simula una distribución realista de los 284,807 registros
    base_clusters = [
    ...
    # Calcular el Cluster 5 de forma que complete el total de muestras
    total_muestras_registradas = sum(c['total'] for c in base_clusters)
    total_fraudes_registrados = sum(c['malicious'] for c in base_clusters)
    
    cluster_5 = {
        'label': 5, 
        'total': max(0, 284807 - total_muestras_registradas), 
        'malicious': max(0, 492 - total_fraudes_registrados)
    }
    
    all_clusters = base_clusters + [cluster_5]

    # Datos de evaluación (Fijos según tus imágenes)
